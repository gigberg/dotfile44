_atuin() {
    ...
    case "${cmd}" in
        atuin)
            opts="-h -V --help --version history import stats search sync login logout register key status account kv default-config server init uuid contributors gen-completions help"
        ...
        atuin__gen__completions)
            ...
            ;;
        atuin__help)
            opts="history import stats search sync login logout register key status account kv default-config server init uuid contributors gen-completions help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        atuin__history__list)
            opts="-c -s -r -f -h --cwd --session --human --cmd-only --print0 --reverse --format --help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 3 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
